...
const AgentManagement: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    agentsApi
      .list({ limit: 100 })
      .then((res) => {
        if (res.data?.length) setRealAgents(res.data);
      })
      .catch(() => {})
  ...
  const rows = useMemo(() => {
    ...
    return agentList.filter((a: any) => {
      ...
      if (
        q &&
        !`${a.name} ${a.code} ${a.phone} ${a.territory} ${a.branch}`.toLowerCase().includes(q)
      )
        return false;
      return true;
    });
  ...
  const columns: TableProps<Agent>['columns'] = [
    ...
    },
    {
      ...
      render: (v: string) => (
        <span className="u-sm u-ink-600">
          <EnvironmentOutlined style={{ marginRight: 6, color: 'var(--ink-400)' }} />
          {v}
        </span>
      ),
    },
    ...
    },
    {
      ...
      width: 90,
      align: 'center',
      render: (v: number) => <span className="tnum u-semibold">{v ?? 0}</span>,
      sorter: (a, b) => (a.sourced ?? 0) - (b.sourced ?? 0),
    },
    {
      title: 'Disbursed',
      dataIndex: 'disbursedValue',
      width: 115,
      align: 'right',
      render: (v: number) => <span className="tnum">{inrCompact(v ?? 0)}</span>,
      sorter: (a, b) => (a.disbursedValue ?? 0) - (b.disbursedValue ?? 0),
    },
    {
      title: 'Rate',
      ...
      width: 80,
      align: 'right',
      render: (v: number) => <span className="tnum">{v ?? 0}%</span>,
    },
    {
      title: 'Commission',
      key: 'commission',
      ...
      render: (_, r) => (
        <div style={{ textAlign: 'right' }}>
          <div className="tnum u-semibold u-success">{inr(r.commissionEarned ?? 0)}</div>
          {(r.commissionPending ?? 0) > 0 && (
            <div className="tnum u-xs u-warning">{inr(r.commissionPending)} pending</div>
          )}
        </div>
      ),
  ...
  const commissionPending = agentList.reduce(
    (s: number, a: any) => s + (a.commissionPending ?? 0),
    0,
  );
  ...
  return (
    <div>
      ...
      <MetricBand
        metrics={[
          { label: 'Active Agents', value: activeCount, sub: `${agentList.length} total` },
          { label: 'Disbursed via Agents', value: inrCompact(totalDisbursed), sub: 'lifetime' },
          { label: 'Commission Paid', value: inrCompact(commissionPaid), sub: 'to date' },
          {
            label: 'Commission Pending',
            value: inrCompact(commissionPending),
            sub: 'accrued, unpaid',
          },
        ]}
      />
      ...
      <Drawer
        title={
          <span>
            <UserAddOutlined style={{ marginRight: 8, color: 'var(--status-info-fg)' }} />
            {editing ? `Edit Agent — ${editing.name}` : 'Onboard Agent'}
          </span>
        }
        size={460}
        open={drawerOpen}
        ...
        footer={
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
            ...
            <Button type="primary" onClick={() => form.submit()}>
              {editing ? 'Save Changes' : 'Onboard Agent'}
            </Button>
          </div>
        }
      >
        <Form form={form} layout="vertical" onFinish={onSubmit} requiredMark={false}>
          {!editing && (
            <Form.Item
              label="Agent Code"
              name="code"
              rules={[{ required: true, message: 'Enter an agent code' }]}
            >
          ...
          <Form.Item
            label="Territory"
            name="territory"
            rules={[{ required: true, message: 'Assign a territory' }]}
          >
          ...
          <Form.Item
            label="Commission Rate (% of disbursed)"
            name="commissionRate"
            rules={[{ required: true, message: 'Set the commission rate' }]}
          >
          ...
          {editing && (
            ...
            >
              <Tag color="blue" style={{ borderRadius: 6 }}>
                Since {fmtDate(editing.joinedOn)}
              </Tag>
              Sourced <strong>{editing.sourced}</strong> · Disbursed{' '}
              <strong>{inrCompact(editing.disbursedValue)}</strong>
            </div>
          )}
        </Form>
      </Drawer>
