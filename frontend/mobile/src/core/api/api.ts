...
async function refreshAccessToken(): Promise<string> {
  ...
  // This bare-axios call bypasses the `api` instance (and its unwrap
  // interceptor), so the backend envelope { success, message, data } must be
  // unwrapped manually here.
  ...
  await secureStorage.set(SECURE_STORE_KEYS.ACCESS_TOKEN, tokens.accessToken);
  await secureStorage.set(SECURE_STORE_KEYS.REFRESH_TOKEN, tokens.refreshToken);
...
api.interceptors.response.use(
  (response) => {
    ...
    // Real services expect the inner payload directly (mocks already return it
    // unwrapped), so flatten the envelope here in one place.
