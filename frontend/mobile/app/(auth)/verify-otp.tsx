...
import { MOCK_OTP, USE_MOCK } from '@/src/core/utils/constants';
...
export default function VerifyOTPScreen() {
  ...
  // Synchronous guard against double submission. `isLoading` is async state, so
  // the auto-submit (OTPInput onComplete) and a manual button tap can both read
  // it as false in the same tick and fire two verify calls — the first consumes
  // the single-use OTP, the second fails. A ref flips synchronously and blocks that.
  ...
  const handleVerify = useCallback(
    async (otpValue: string) => {
      if (!phone || otpValue.length !== 6 || isLoading || submittingRef.current) return;
      ...
      try {
        ...
        // Re-open the guard so the user can retry after a recoverable error
        // (wrong code, expired OTP, network). On success we navigate away and
        // intentionally leave the guard closed.
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        >
          ...
          {__DEV__ && USE_MOCK && Boolean(MOCK_OTP) && (
